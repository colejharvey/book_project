...
models2 <- list()
models2[['Model 1']] <- model_dev_ethnic
models2[['Model 2']] <- model_dev_pctrus
models2[['Model 3']] <- model_dev_regcomp
models2[['Model 4']] <- model_spike_ethnic
models2[['Model 5']] <- model_spike_pctrus
models2[['Model 6']] <- model_spike_regcomp

library(kableExtra)

reg_table <- modelsummary(models2, output = "kableExtra",  stars = T, title = 'Table 1: OLS models of election falsification', gof_omit = c("Log|RMSE|AIC|BIC"), coef_map = c("(Intercept)" = "(Intercept)",
...
"presidential" = "Presidential election",                                                                 "pensioners_per_1000" = "Pensioners per 1000 pop.",                                                                 "grp_by_pop_small" = "GRP per capita",                                                                   "poverty" = "Poverty rate", 
  ...
  add_header_above(c(" " = 1, "Deviance measure" = 3, "RKD (spikes) measure" = 3)) 
...
Non-standard is next.

```{r echo=FALSE, warning=FALSE, message=FALSE}
model_ns_ethnic <- lm(nonstandard_coef_ur ~  ethnic_status +  ur_gov_share + subnational_party_control + population   + presidential +pensioners_per_1000 + grp_by_pop +  poverty + urban_pct   , data = full_data)
summary(model_ns_ethnic)



model_ns_pctrus <- lm(nonstandard_coef_ur ~  pct_russian_upd  + ur_gov_share + subnational_party_control    + presidential +pensioners_per_1000 + grp_by_pop +  poverty + urban_pct  + population  , data = full_data)
summary(model_ns_pctrus)


model_ns_regcomp <- lm(nonstandard_coef_ur ~  regional_comp  + ur_gov_share + subnational_party_control    + presidential +pensioners_per_1000 + grp_by_pop +  poverty + urban_pct  + population  , data = full_data)

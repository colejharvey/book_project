...
The main text focuses on elections where the incumbent claims victory. However, classic regime-weakness models also predict that some incumbents who will voluntarily avoid electoral manipulation and accept a loss at the ballot box. If this is the case, it could introduce selection bias into the sample for the duration analyses in the main paper by sorting only the strongest incumbents into the sample that faces protest.

Fully investigating this possibility should be the focus of future work, but some initial analyses are provided here to show that this is not the case. The analysis in this subsection proceeds in three steps. First, using all the observations in the dataset\textemdash winning and losing incumbents\textemdash I estimate the predicted level of election fraud for each case, using only structural variables (i.e. professionalization of the bureaucracy, electoral management body autonomy, urbanization, etc) in an OLS model. Factors associated with deterrence, like the number of pre-election protests are excluded. This should produce an estimate of the level of manipulation the incumbent could produce in the absence of strategic deterrence.

Second, the residuals from this first model are then used as a dependent variable in second model. The residuals reflect the degree to which an incumbent over- or under-produced manipulation, relative to their expected baseline. If the deterrence interpretation is correct (which may lead to selection effects), the indicators of protest risk should be negatively associated with these residuals. In other words, higher protest risk should lead incumbents to under-produce manipulation. Under the regime-strength logic, we might expect the opposite, as incumbents work hard to build and indicate strength in riskier environments. This model is also an OLS model.

Lastly, two logit models of incumbent loss are presented, using the fitted values from the first model and indicators of protest risk as predictors. The fitted values indicated the expected, structural level of electoral manipulation. Here, the deterrence logic would expect that indicators of pre-election protest risk will make incumbents more likely to lose, while the regime-strength model would expect that incumbent loss will be largely driven by structural factors.   
...
Models 7 and 8 take the fitted values from the first model and employ them as an explanatory variable for incumbent loss, while controlling for pre-election protest. As both models show, the more incumbents invest in the structural prerequisites of manipulation, the less likely they are to lose elections. By contrast, we observe little or no effect for pre-election protest risk. Model 7 shows no effect for regional protest diffusion on incumbent loss, while Model 8 shows a significant (p = 0.05) but substantively small effect for pre-election protest.  

All told, this analysis supports the argument in the main text, which holds that the capacity to manipulate heavily is associated with incumbent durability. The results in this section are consistent with a world in which losing incumbents are not voluntarily surrendering power in the face of a committed opposition (on average), but are failing to produce sufficient election manipulation to win due to structural constraints. This should reduce the concern that the main findings are driven by an endogenous process, in which incumbents who face more organized and capable opposition actors are strategically avoiding protest by producing less fraud.
...
models2[['Model 7']] <- model.lose_reg
models2[['Model 8']] <- model.lose_events

row <- data.frame(cbind("", "Fraud", "Fraud residual", "Inc. loss", "Inc. loss"))
